const { getHealthData1,handleNutrients,handleRecipe } = require('../controlles/handlers');
...
describe('Test Pure Functions',()=>{
	...
	test('handleNutrients function should return object have these keys [ENERC_KCAL, PROCNT, FAT, SUGAR, CHOCDF, CHOLE ] only and their data',()=>{
		const obj = {
			...
			FAMS: { label: 'Monounsaturated', quantity: 32.422316318640384, unit: 'g' },
			...
			'SUGAR.added': { label: 'Sugars, added', quantity: 6.137700000000001, unit: 'g' },
			...
			VITA_RAE: { label: 'Vitamin A', quantity: 407.25119695914213, unit: 'µg' },
			VITC: { label: 'Vitamin C', quantity: 69.33900700043584, unit: 'mg' },
			THIA: { label: 'Thiamin (B1)', quantity: 7.580114662552134, unit: 'mg' },
			RIBF: { label: 'Riboflavin (B2)', quantity: 3.40290136455282, unit: 'mg' },
			...
			FOLFD: { label: 'Folate (food)', quantity: 485.50313659586453, unit: 'µg' },
		...
		const actual = handleNutrients(obj);
		const expected = {
			ENERC_KCAL: { label: 'Energy', quantity: 2620.983838835843, unit: 'kcal' },
			PROCNT: { label: 'Protein', quantity: 73.06396909756897, unit: 'g' },
			FAT: { label: 'Fat', quantity: 70.85852721973856, unit: 'g' },
